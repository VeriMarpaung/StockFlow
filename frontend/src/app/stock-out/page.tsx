...
export default function StockOutPage() {
  ...
  const loadProducts = () =>
    api.get('/products').then((r) => {
      ...
      setProducts(list);
      // Refresh selected product's version after reload
      return list;
    });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setQuantity(''); setNote('');
      await loadProducts(); // Refresh version after success
    } catch (err) {
      if (isAxiosError(err)) {
        if (err.response?.status === 409) {
          // Race condition detected — reload product to get fresh version
    ...
    <AppLayout>
      ...
      <div className="bg-white rounded-lg shadow-sm p-6 max-w-md">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Conflict handling UI — key for demo */}
          {conflictMsg && (
